...
export default function Popupcart({ data, passclose, active }) {
  const transitions = useTransition(active, {
    ...
    config: { duration: 100, tension: 280, friction: 60 },
  });
  ...
  return (
    <>
      {active && (
        <div
          onClick={passclose}
          className="fixed z-30 top-0 left-0 bg-zinc-800/50 h-full w-full"
        />
      )}
      {transitions((style, i) => (
        <a.div
        ...
        >
          <div>
            ...
            <div className="divide-y max-h-[470px] overflow-y-scroll">
              {data.map((item, i) => (
                <div key={i} className="flex py-5 items-start gap-5">
                  ...
                  <div className="flex justify-between flex-col gap-1">
                    ...
                    <div className="text-[15px]">
                      <p>Device: {item?.optionalData.type}</p>
                      <p>Model: {item?.optionalData.model}</p>
                      <p>Version: {item?.optionalData.version}</p>
                    </div>
          ...
          <div className="flex flex-col gap-3">
            <Link href="/cart">
              <a
                ...
                className="hover:bg-zinc-800 border border-zinc-800 text-center text-zinc-800 hover:text-white w-full px-5 py-4 rounded hover:bg-zinc-700 font-semibold md:text-xl duration-100 ease-linear"
              >
                View Cart
              </a>
            </Link>
